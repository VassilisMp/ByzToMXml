...
public class Scale {
    ...
    static class CyclicIterator <E> {
        ...
        E getNext() {
            ...
            /*final E element = list.get(cursorPos++);
            cursorPos %= list.size();
            return element;*/
        }
        ...
        E getItemToRight(int num) {
            cursorPos = (cursorPos+num)%list.size();
            return list.get(cursorPos);
        }

        E getItemToLeft(int num) {
            cursorPos = list.size()-((cursorPos+num)%list.size());
            return list.get(cursorPos);
        }

        public void resetCursor(int cursorPos) {
            this.cursorPos = 0;
        }
    }

    static {
        final LinkedList<Martyria> scale = new LinkedList<>();
        ...
        final CyclicIterator<Martyria> iterator = new CyclicIterator<>(martyrias);
        iterator.getItemToLeft(2);
        for (Martyria martyria : martyrias)
            martyria.commasToPrev = iterator.getNext().commasToNext;
        List<Martyria> copy = martyrias.stream().map(Martyria::new).collect(Collectors.toList());
        copy.forEach(martyria -> martyria.octave+=1);
    }
}
